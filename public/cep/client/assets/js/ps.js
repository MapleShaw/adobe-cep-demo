try {
  ...
  console.log('=====gExtensionID=====', gExtensionID)
  ...
  function PhotoshopCallbackUnique (csEvent) {
    try {
      if (typeof csEvent.data === 'string') {
        var eventData = csEvent.data.replace('ver1,{', '{') // 特定字符串处理
        ...
        if (eventDataParse.eventID === eventMake) { uiItemToUpdate = lblMake } else if (eventDataParse.eventID === eventDelete) { uiItemToUpdate = lblDelete } else if (eventDataParse.eventID === eventClose) { uiItemToUpdate = lblClose } else if (eventDataParse.eventID === eventSelect) { uiItemToUpdate = lblSelect } else if (eventDataParse.eventID === eventSet) { uiItemToUpdate = lblSet }

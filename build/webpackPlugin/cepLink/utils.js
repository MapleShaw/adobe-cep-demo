...
const symlinkExtension = (path, BUNDLE_ID) => {
  ...
  const target = getSymlinkExtensionPath(BUNDLE_ID)
...
const enablePlayerDebugMode = () => {
  if (process.platform === 'win32') {
    execSync('REG ADD HKCU\\Software\\Adobe\\CSXS.9 /f /v PlayerDebugMode /t REG_SZ /d 1')
    execSync('REG ADD HKCU\\Software\\Adobe\\CSXS.9 /f /v LogLevel /t REG_SZ /d 6')
  } else {
    execSync('defaults write com.adobe.CSXS.9 PlayerDebugMode 1')
    execSync('defaults write com.adobe.CSXS.9 LogLevel 6')
  }
}

// open the debugger page in chrome
// 由于 chrome 新版本打开调试页面会报错，就不采用这种调试方式
